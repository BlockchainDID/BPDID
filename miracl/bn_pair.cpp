...
//Hash 2
void PFC::start_hash2(void)
{
	sha3_init(&SH2, 32);
}

Big PFC::finish_hash_to_group2(void)
{
	Big hash;
	char s[HASH_LEN];
    sha3_hash(&SH2,s);
...
Big PFC::finish_hash_to_aes_key2(void)
{
	Big hash;
	char s[HASH_LEN];
    sha3_hash(&SH2,s);
...
void PFC::add_to_hash2(const GT& x)
{
	ZZn4 u;
	ZZn12 v=x.g;
	ZZn2 h,l;
...
void PFC::add_to_hash2(const G2& x)
{
	ZZn2 X,Y;
	ECn2 v=x.g;
	Big a;
...
void PFC::add_to_hash2(const G1& x)
{
	Big a,X,Y;
	int i,m;
	x.g.get(X,Y);
...
void PFC::add_to_hash2(const Big& x)
{
	int m;
	Big a=x;
    while (a>0)
...
void PFC::add_to_hash2(char *x)
{
	int i=0;
	while (x[i]!=0)
	{
		sha3_process(&SH2,x[i]);
		i++;
	}
}

//Hash 3
void PFC::start_hash3(void)
{
	shs512_init(&SH3);
}

Big PFC::finish_hash_to_group3(void)
{
	Big hash;
	int i,j;
	char s[64];
...
/*
Big PFC::finish_hash_to_aes_key3(void)
{
	Big hash;
	char s[HASH_LEN];
    shs512_hash(&SH3,s);
...
*/
void PFC::add_to_hash3(const GT& x)
{
	ZZn4 u;
	ZZn12 v=x.g;
	ZZn2 h,l;
    ...
    for (i=0;i<4;i++)
    {
        ...
        {
            m=a%256;
            shs512_process(&SH3,m);
            a/=256;
        }
    }
}

void PFC::add_to_hash3(const G2& x)
{
	ZZn2 X,Y;
	ECn2 v=x.g;
	Big a;
...
void PFC::add_to_hash3(const G1& x)
{
	Big a,X,Y;
	int i,m;
	x.g.get(X,Y);
...
void PFC::add_to_hash3(const Big& x)
{
	int m;
	Big a=x;
    while (a>0)
...
void PFC::add_to_hash3(char *x)
{
	int i=0;
	while (x[i]!=0)
	{
		shs512_process(&SH3,x[i]);
		i++;
	}
}
